...
; modifies a, x, y
.macro graphics_vload_block src_addr, dest, size
    lda #V_INC_1
...
; modifies a, x, y
.macro graphics_vload_palette src_addr, start, size
    lda #start
    ...
    jsr _graphics_dma_palette
.endmacro

; Load Palette with DMA
; In A:X - points to the data (bank and address)
; Y - size of data
_graphics_dma_palette:
    phb  ; store bank
    php  ; store processor status registers
...
graphics_vload_sample_palette:
    ...
    lda #PAL_SAMPLE_ADDR        ; according to A-17 in OBJ palettes in mode 0
    sta CGADD
